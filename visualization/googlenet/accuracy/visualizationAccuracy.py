...
dataACA = load('HmobileACAaccucuracy.npy')
HMatrixrank=np.array(read_mat('HMatrixSVDrank.mat')['v'])
figure(figsize=(30, 20), dpi=80)
matplotlib.rc('xtick', labelsize=25) 
matplotlib.rc('ytick', labelsize=25) 
AccwithoutApprox=np.ones(dataSVD.size)*dataSVD[0]
AccwithoutApprox=np.delete(AccwithoutApprox,0)
dataQR=np.delete(dataQR,0)
dataSVD=np.delete(dataSVD,0)
dataACA=np.delete(dataACA,0)
print(AccwithoutApprox)
print(dataSVD)
plt.plot(HMatrixrank,dataACA,linewidth=8.0,label='ACA',linestyle='-.')
plt.plot(HMatrixrank,dataQR,linewidth=8.0,label='QR',linestyle='--')
plt.plot(HMatrixrank,dataSVD,linewidth=8.0,label='SVD',linestyle=':')
